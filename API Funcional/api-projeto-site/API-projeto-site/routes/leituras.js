...
router.get('/ultimas/:idcaminhao', function(req, res, next) {
	...
	if (env == 'dev') {
		...
		instrucaoSql = `select LEITURA_UMIDADE, LEITURA_DATA_HORA, DATE_FORMAT(LEITURA_DATA_HORA,'%H:%i:%s') as momento_grafico from HISTORICO_SENSOR where FK_SENSOR = ${idcaminhao} order by ID_HISTORICO desc`;
...
router.get('/tempo-real/:idcaminhao', function(req, res, next) {
	...
	if (env == 'dev') {
		...
		// abaixo, escreva o select de dados para o SQL Server
		instrucaoSql =`select LEITURA_UMIDADE, LEITURA_DATA_HORA, DATE_FORMAT(LEITURA_DATA_HORA,'%H:%i:%s') as momento_grafico, FK_SENSOR from HISTORICO_SENSOR where FK_SENSOR = ${idcaminhao} order by ID_HISTORICO desc`;
