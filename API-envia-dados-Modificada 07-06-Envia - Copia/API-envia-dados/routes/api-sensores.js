...
router.get("/sendData", (request, response) => {
	...
	data_atual = new Date();
	data_atual_completa = `'${data_atual.getFullYear()}-${data_atual.getMonth()}-${data_atual.getDate()} ${data_atual.getHours()}:${data_atual.getMinutes()}:${data_atual.getSeconds()}'`;
	var instrucaoSql = "";

	var sensor = parseInt(Math.random() * (1004 - 1000) + 1000);
	...
	if (env == "dev") {
		...
		values (${sensor},${Humidity},${data_atual_completa})`;
	}
